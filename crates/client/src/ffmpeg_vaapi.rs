...
impl MpegTsH264Sink {
    pub fn new(url: &str, muxer: &str, width: u32, height: u32) -> Result<Self> {
        let url_c = cstring(url)?;
        let muxer_c = cstring(muxer)?;
        ...
        unsafe {
            check(
                ...
                "allocating FFmpeg output context",
            )?;
            if ctx.is_null() {
                bail!("allocating FFmpeg output context returned null");
            }

            let stream = ffi::avformat_new_stream(ctx, ptr::null());
            if stream.is_null() {
                ffi::avformat_free_context(ctx);
                bail!("creating FFmpeg H.264 output stream failed");
            }
            ...
            if codecpar.is_null() {
                ffi::avformat_free_context(ctx);
                bail!("FFmpeg output stream has no codec parameters");
            }
            (*codecpar).codec_type = ffi::AVMediaType::AVMEDIA_TYPE_VIDEO;
            (*codecpar).codec_id = ffi::AVCodecID::AV_CODEC_ID_H264;
            (*codecpar).codec_tag = 0;
            (*codecpar).width = width.max(1) as i32;
            (*codecpar).height = height.max(1) as i32;
            (*codecpar).format = ffi::AVPixelFormat::AV_PIX_FMT_YUV420P as i32;

            if !(*ctx).oformat.is_null() && ((*(*ctx).oformat).flags & ffi::AVFMT_NOFILE) == 0 {
                check(
                    ffi::avio_open2(
                        &mut (*ctx).pb,
                        url_c.as_ptr(),
                    ...
                    "opening FFmpeg output URL",
                )?;
            }
            check(
                ffi::avformat_write_header(ctx, ptr::null_mut()),
                "writing FFmpeg output header",
            )?;
            ...
            info!(
                ...
                width = width.max(1),
                height = height.max(1),
                "opened FFmpeg MPEG-TS H.264 output"
            );
    ...
    pub fn write_access_unit(
        ...
        if payload.len() > i32::MAX as usize {
            bail!(
                "H.264 access unit is too large for FFmpeg packet: {}",
                payload.len()
            );
        }
        unsafe {
            ...
            if packet.is_null() {
                bail!("allocating FFmpeg output packet failed");
            }
            (*packet).data = payload.as_ptr() as *mut u8;
            (*packet).size = payload.len() as i32;
            (*packet).stream_index = self.stream_index;
            let duration = (duration_ms.max(1) as i64).saturating_mul(90);
            let pts = pts_ms.saturating_mul(90).max(self.next_pts);
            self.next_pts = pts.saturating_add(duration);
            (*packet).pts = pts;
            ...
            let ret = ffi::av_interleaved_write_frame(self.ctx, packet);
            (*packet).data = ptr::null_mut();
            (*packet).size = 0;
            ffi::av_packet_free(&mut packet);
            check(ret, "writing FFmpeg H.264 packet")
        }
    }

    pub fn finish(&mut self) -> Result<()> {
        if self.trailer_written || self.ctx.is_null() {
            return Ok(());
        }
        ...
        unsafe {
            check(
                ...
                "writing FFmpeg output trailer",
            )
        }
    }
}

impl Drop for MpegTsH264Sink {
    fn drop(&mut self) {
        if self.ctx.is_null() {
            return;
        }
        if !self.trailer_written {
            unsafe {
                let _ = ffi::av_write_trailer(self.ctx);
            }
            self.trailer_written = true;
        }
        unsafe {
            if !(*self.ctx).pb.is_null()
                && !(*self.ctx).oformat.is_null()
                && ((*(*self.ctx).oformat).flags & ffi::AVFMT_NOFILE) == 0
            {
                let _ = ffi::avio_closep(&mut (*self.ctx).pb);
            }
